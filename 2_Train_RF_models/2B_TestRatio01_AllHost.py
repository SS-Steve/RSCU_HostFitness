...
Study_Title = f"{Dataset_Note}_TestRatio01_AllHosts"

CPU_USED = os.cpu_count() - 1
...
Optuna_CV_Score = 'balanced_accuracy'
...
score_savename_ref = {'balanced_accuracy':'BA',
                      'f1':'F1', 
                      'recall':'Recall'}
NOTING = f"[AllHosts]_for{score_savename_ref[Optuna_CV_Score]}"
OUTPUT_ADD_NOTE = (
...
FINAL_RESULT = {}
Run_List = animal_list[:]

for Index, POS_LABEL in enumerate(Run_List):
    ...
    print(f"{POS_LABEL_Raw} - Finish Train-test split.")
    ...
    print(f"{POS_LABEL_Raw} - Finish resampling.")
    print(f"{POS_LABEL_Raw} - Optimising starts.")
    ...
    study = optuna.study.create_study(study_name=POS_LABEL, direction='maximize')
    study.optimize(objective, n_trials=Optuna_Trial_N, n_jobs=CPU_USED)


    print (f"{POS_LABEL_Raw} - Done hyper-parameters tuning.")
    ...
    ss.database_save(study, f"{FILENAME_OUTPUT}//study_{POS_LABEL_Raw}", 'Output')
    study._storage.delete_study(study._study_id)
    ss.save_file(DF_OUTPUT, f"{FILENAME_OUTPUT}//DF_OUTPUT")

    print (f"{POS_LABEL_Raw} - Results saved.")
    
    
print ('All Optimisation Done!')


# nohup python Optuna_Optimise_TestRatio01_AllHost.py > Optuna_Optimise_TestRatio01_AllHost.log 2>&1 &

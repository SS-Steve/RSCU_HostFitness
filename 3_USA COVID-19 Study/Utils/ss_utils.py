...
def codon_usage_to_RSCU_usage(codon_usage_dict={}):
    ...
    for i in codon_usage_dict:
        ...
        if codon_usage_dict[i] == 0.0:
            ...
            rscu = (codon_usage_dict[i] / aa_usage_dict[aa]) * len(translation_aa_codon_dict[aa])
        dict_output[i] = rscu
    return dict_output
...
def discriptive_data(in0, raw_output=False):
    ...
    if isinstance(in0, dict) == True:
        ...
        try:
            ...
            pass
    if len(list_data) == 0:
        dict_describe = {'count': None, 'sum'     : None, 'mean'    : None, 'median'          : None, 'variance': None,
                         'std'  : None, 'min'     : None, 'max'     : None, '25%'             : None, '50%'     : None,
                         '75%'  : None, 'skewness': None, 'kurtosis': None, 'weighted_average': None, 'gmean_0' : None}
    else:

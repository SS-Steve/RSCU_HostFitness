...
Study_Title = f"{Dataset_Note}_LOO_FalseReRun"
...
# Extract samples with wrong predictions
loo_results_filename = f"LOO_Results//{Dataset_Note}_LOO_FinalResults_for{score_savename_ref[Optuna_CV_Score]}.csv"
df_loo_results = pd.read_csv(loo_results_filename)
False_Index_List = df_loo_results[(df_loo_results['[Test]_FP'] == 1) | (df_loo_results['[Test]_FN'] == 1)]['Sample_Index'].tolist()

FINAL_RESULT = {}

# Run_List = Run_List_Total[Run_List_Index_Comb[0]: Run_List_Index_Comb[1]] # Get a part of the total list
Run_List = False_Index_List[:]
# Run_List = Run_List[1:10] # for code testing
print (f"Total Run: {len(Run_List)}")
...
for train_test_split_iter_n in Run_List:
    ...
    OUTPUT.update({'y_test':y_test[0], 
                   'not_human_pp':y_pred_proba[0], 
                   'human_pp':y_pred_proba[1]})
